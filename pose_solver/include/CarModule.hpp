                            ...
                            double delta_time)
{
    ...
    do {
        ...
        img(get_y(999 - (int)(error_sum*2)), 2*cnt) = 255;
        gradient_sum *= 10;
        img(get_y(999 - (int)((old_error - error_sum)*2e2)), get_x((int)(gradient_sum.norm()*2e5))) = 200;
        cnt++;
        if (cnt > 500)
            break;
    } while (abs(error_sum - old_error) > 0.00001);
    cv::imshow("error", img);
    cv::waitKey(1);
