 ...
 *      采用非旋转矩形进行标定（四边形匹配）
 *      其中pnpTarget的定义有所改变，包含的是一个四个点的数组（一个Point2f数组指针）
...
public:
    GetPos pos_getter;
    std::vector<pnpTarget> tar_list;                                       //目标装甲板
...
std::vector<cv::RotatedRect> lights)
{
    ...
    #endif
    //print("pnpTarget list size(%d), valid size(%d).\n", tar_list.size(), _cnt);
}
...
void ArmorPlate::match(cv::RotatedRect r1, cv::RotatedRect r2, int pos1, int pos2){
    ...
    if(isRatioValid() && isEdgesValid()){
        print("Pushed in.(%d, %d).\n", pos1, pos2);
        //数字识别预留位置！！！！！！！！！！！！！！！！！
        tar_list.push_back(pnpTarget(points, pos1, pos2, 0));
    }
    else{
