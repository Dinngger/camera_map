...
private:
    ...
    static bool isValidRatio(cv::RotatedRect rect);         //装甲板的长宽比合适吗
...
void ArmorPlate::match(cv::RotatedRect r1, cv::RotatedRect r2, int pos1, int pos2){
    ...
    if(isValidRatio(rect)){
        print("Pushed in.(%d, %d).\n", pos1, pos2);
        tar_list.push_back(Target(rect, pos1, pos2, 0));
    }
    else{
...
//-----------------------------------------------------------
bool ArmorPlate::isValidRatio(cv::RotatedRect rect){
