...
def enhance(data: np.array, num: int):
    res = data
    for _ in tqdm(range(num)):
        data_ = randomShift(data)
        data_ = mirroData(data_)
        data_ = randomDelete(data_)
        res = np.vstack((res, data_))
    return res
...
def visualize(data: np.array, start_frame=0):
    ...
    for frame, row in enumerate(data):
        ...
        cv.putText(img, str(frame), (5, 15),
                   cv.FONT_HERSHEY_SIMPLEX, 0.75, colors[3], 2)
        for i in range(max_light):
            if int(row[pre_base + i]) == 0:
                continue
            center = row[4 * i: 4 * i + 2]
            ...
            cv.putText(img, str(i), tuple(pos_text),
                       cv.FONT_HERSHEY_SIMPLEX, 0.75, colors[3], 1)
...
if __name__ == "__main__":
    ...
    if number == 0:
        ...
        result = enhance(data, 100)
        with open(enhanced_path, 'w') as file:
